...
class PatientData(GSTTData):
    ...
    def get_CBCT_relative_timepoints(self):
        ...
        CBCT_dates = [date(int(CBCT_date[0:4]), int(CBCT_date[4:6]),  int(CBCT_date[6:8])) for CBCT_date in self.CBCT_dates]
...
class Image(GSTTData):
    ...
    def rename_parotid(self, name, flip = False):
        new_name = name
        file_path = self.base_path + '/' + self.PatientNo + '/CT_' + str(self.pCT_date) + '/STRUCTURES/'
    ...
    def mask_CT(self, img_path):
        ...
        couch_path = self.base_path + '/' + self.PatientNo + '/CT_' + str(self.pCT_date) +'/STRUCTURES/BIN_COUCH.nii.gz'
        masked_CT_path = self.base_path + '/' + self.PatientNo + '/CT_' + str(self.pCT_date)  + '/MASKED_CT.nii.gz'

        # read in data 
        img_data, img_affine, img_header = self.get_img_objects(img_path)
        print(couch_path)
        couch_slice = self.get_couch_slice(couch_path)
        print(couch_path)
    ...
    def mask_anatomy(self):
        ...
        masked_img_path = self.base_path + '/' + self.PatientNo + '/CT_' + str(self.pCT_date) + '/MASKED_CT.nii.gz'
        ...
        masked_img = self.base_path + '/' + self.PatientNo + '/CT_' + str(self.pCT_date) + '/atlas_MASKED_CT.nii.gz'
    ...
    def mask_CBCT(self, input_CBCT_path, slices_above, mask_couch = True):
        ...
        # reads in CBCT 
        img_data, img_affine, img_header = self.get_img_objects(input_CBCT_path, data_type= np.float32)
        img_data_copy = img_data.copy()
        l, w, h = np.shape(img_data_copy)
        ...
        if mask_couch:
            ...
            boolean = (body_img_data == 0) & (img_data_copy != np.NaN) & (img_data_copy >= -950) 
            ...
            for i in np.arange(5,9)[::-1]:
                ...
                if np.sum(couch_mask_new) != 0:
                    ...
                    # mask out the couch 
                    img_data_copy[:,yslice:,:] = np.NaN
                    break
        else:
            pass

        # mask for the circular field of view 
        mask = np.zeros(shape = np.shape(img_data_copy))
        
        # creates a circular mask with radius = h/2 
        # assumes the slices are square
        for slice in np.arange(0, np.shape(img_data_copy)[2]):
            mask[:,:, slice] = self.create_circular_mask(l,w)

        img_data_copy[mask == 0] = np.NaN
        img_data_copy[:,:,0:slices_above] = np.NaN
        img_data_copy[:,:,h-slices_above:h] = np.NaN

        masked_CBCT_path = input_CBCT_path[:-11] + '/MASKED_' + input_CBCT_path[-11:]
        img_data_copy = np.array(img_data_copy, dtype = np.float32)
        NewNiftiObj = nib.Nifti1Image(img_data_copy, img_affine, img_header)
...
class GroupwiseRegs(GSTTData):
    ...
    def avgImage(self):
        ...
        # average each slice individually since numpy was complaining about memory
        ...
        #Nan_map = np.empty(shape=(img_shape[0],img_shape[1],len(self.resampled_imgs)))
        ...
        for slice in np.arange(0, img_shape[2]):
            ...
            #Masked_Average = np.empty(shape = (img_shape[0],img_shape[1],img_shape[2]))
            #for slice in np.arange(0, img_shape[2]):
            Masked_Average[:,:,slice] = np.nanmean(Nan_map, axis = 2)
        del(Nan_map)
...
class AtlasRegs(GSTTData):
    ...
    def ResampleImgs(self, CBCT_timepoints):
        ...
        float_img = self.PatientCTPath + 'MASKED_CT.nii.gz'
        resampled_img = self.PatientUCLHRegsPath + '/MASKED_CT.nii.gz'
